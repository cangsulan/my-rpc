...
public class ZooKeeperRegistry implements Registry {
    ...
    public List<ServiceMetaInfo> serviceDiscovery(String serviceKey) {
        ...
        try {
            ...
            log.debug("key："+serviceKey+" 被写入到了caffeine本地缓存中");
     ...
     * 监听（消费端）
     *
     * @param serviceKey 服务节点 key
     */
    @Override
    public void watch(String serviceKey) {
        // 使用 前缀 + serviceKey 来监听，而不是 serviceNodeKey
            ...
            curatorCache.listenable().addListener(
                    CuratorCacheListener
                            ...
                            .forDeletes(childData -> {
                                // JSON反序列化，提取出节点信息，然后得到 serviceKey，进行缓存删除操作
                                ServiceMetaInfo info = JSONUtil.toBean(Arrays.toString(childData.getData()), ServiceMetaInfo.class);
                                registryServiceCache.invalidate(info.getServiceKey());
                            })
                            .forChanges(((oldNode, node) -> {
                                // JSON反序列化，提取出节点信息，然后得到 serviceKey，进行缓存删除操作
                                ServiceMetaInfo info = JSONUtil.toBean(Arrays.toString(oldNode.getData()), ServiceMetaInfo.class);
                                registryServiceCache.invalidate(info.getServiceKey());
                            }))
                            .build()
            );
